...
object BulkFilesInsertToOracleInsert {
  ...
  def dataCatalog(): DataFrame = {
    // Option 'dbtable' or 'query' is required.
    //val query ="SELECT * FROM (select * FROM AAP_WELCOME_T2  limit 1) e WHERE 1=0";
    val pushdown_query = "(select * from AAP_WELCOME_T2) emp_alias"
    val empDF = getSparkSession().read.format("jdbc")
  ...
  def main3(args: Array[String]): Unit = {
    ...
    val empDF = getSparkSession().read.format("jdbc")
      .option("url", "jdbc:oracle:thin:@localhost:1521:xe")
      .option("dbtable", "AAP_WELCOME_T")
      .option("user", "sys as sysdba")
      .option("password", "welcome")
      .option("driver", "oracle.jdbc.driver.OracleDriver")
      .load()

    empDF.coalesce(1).write.format("csv").option("header", "true").save("C:/data/spark_output/ops");

    //empDF.printSchema()
    //empDF.show(10,false)
    //print(empDF.schema)
    //print(empDF.columns)
    //writeOperation(empDF)
    //
    //    val df2=emptyDataFrame();
    //    df2.printSchema()
